...
var markdown = require('./marked').marked;
var response = (!COMMON_NODE) ? require('ringo/jsgi/response') : {};
...
var mcms = exports.mcms = function(root, options) {
  root = root || fs.workingDirectory();
  options = options || {};
  var PAGES = fs.resolve(root, './pages');
  var TEMPLATES = fs.resolve(root, './templates');
  var INCLUDES = fs.resolve(root, './includes');
  var STATIC = fs.resolve(root, './static');
